...
import { getEvaluationCategories, getColorByIndex } from '@/lib/data-processor';
import { categoryToFieldMap } from '@/lib/chart-utils';

interface EvaluatorViewProps {
    evaluatorData: EvaluationItem[] | null;
    evaluators: string[];
    selectedEvaluator: string;
    onEvaluatorSelect: (evaluator: string) => void;
}
...
export function EvaluatorView({
    ...
    useEffect(() => {
        if (!svgRef.current || !evaluatorData || evaluatorData.length === 0) return;

        renderEvaluatorChart(svgRef.current, evaluatorData);
    ...
    const renderEvaluatorChart = (svg: SVGSVGElement, data: EvaluationItem[]) => {
        // 차트 크기 설정
        ...
        // 단, 최대값이 5 이하면 5를 사용하고, 5보다 크면 실제 최대값을 사용
        ...
        data.forEach((item, idx) => {
            const jobName = `${item.직군}-${item.직렬}-${item.직무}`;
            ...
            const lineData = categories.map(category => {
                ...
                const value = item[fieldName] as number;
        ...
        // 직무 범례
        data.forEach((item, idx) => {
            const jobName = `${item.직무}`;  // 직무명만 표시 (간략화)
        ...
        const avgLegend = legend.append('g')
            .attr('transform', `translate(0, ${data.length * 20 + 10})`);
        ...
        <>
            <Card className="mb-4">
                <CardContent className="p-4">
                    <div>
                        <label className="block text-sm font-medium mb-1">평가자 선택</label>
                        <Select value={selectedEvaluator} onValueChange={onEvaluatorSelect}>
                            <SelectTrigger>
            ...
            {evaluatorData && evaluatorData.length > 0 && (
                <Card className="w-full">
                    <CardHeader>
                        <CardTitle>{selectedEvaluator}님의 직무별 평가 점수</CardTitle>
                    </CardHeader>
                    <CardContent>
